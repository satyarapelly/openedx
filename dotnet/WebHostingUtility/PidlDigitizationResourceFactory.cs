...
namespace Microsoft.Commerce.Payments.PidlFactory.V7
{
    ...
    public sealed class PidlDigitizationResourceFactory
    {
        ...
        private static List<PIDLResource> GetTermsAndConditions(PaymentInstrument digitizedCard, string partnerName, EventTraceActivity traceActivityId = null)
        {
            ...
            if (pendingDetails != null)
            {
                ...
                if (string.IsNullOrEmpty(mimeType) || mimeType == "text/plain")
                {
                    ...
                    {
                        displayHintTermsAndConditions.DisplayContent = GetTermsAndConditionsText(traceActivityId, termsAndConditionsUrl);
                    }
                    else
                    {
        ...
        private static RestLink GetSelfRestLink()
        {
            ...
            // The self link has to be same as the current url. Without overriding the self, the self URL would be http://localhost/px/v4/challenge-descriptions?type=termsAndConditions&language=en-US // lgtm[cs/non-https-url] Suppressing Semmle warning
            // whereas the self URL should be http://localhost/px/v4/challengedescriptions?pendingoperation={caid}/paymentinstruments/{piid}/operations/{opid} // lgtm[cs/non-https-url] Suppressing Semmle warning
            RestLink selfLink = new RestLink() { Href = currentUrl.ToString(), Method = Constants.HTTPVerbs.GET };
            return selfLink;
        }
        ...
        private static string GetTermsAndConditionsText(EventTraceActivity traceActivityId, string termsAndConditionsUrl)
        {
            string termsText = string.Empty;
            List<KeyValuePair<string, string>> headers = new List<KeyValuePair<string, string>>();
            headers.Add(new KeyValuePair<string, string>(HttpRequestHeader.Accept.ToString(), GlobalConstants.HeaderValues.TextContent));
            GenericHttpClient.SendAndReceive(
