    ...
    public class CertificateContainer
    {
        ...
        public bool GrantNetworkServiceRead()
        {
            ...
            try
            {
                ...
                Logger.Log("Locating private key file...");
                string keyFileName = rsa.CspKeyContainerInfo.UniqueKeyContainerName;
                string keyFilePath = Path.Combine(SE.GetFolderPath(SE.SpecialFolder.CommonApplicationData), @"Microsoft\Crypto\RSA\MachineKeys", keyFileName);

                if (!File.Exists(keyFilePath))
                {
                    Logger.Log("Fallback to user profile directory.");
                    string altDir = Path.Combine(SE.GetFolderPath(SE.SpecialFolder.ApplicationData), @"Microsoft\Crypto\RSA");
                    string[] matching = Directory.GetFiles(altDir, keyFileName, SearchOption.AllDirectories);
                    if (matching.Length == 0)
                    {
                        Logger.Log("Key file not found.");
                        return false;
                    }
                    keyFilePath = matching[0];
                }

                Logger.Log("Modifying ACL for: {0}", keyFilePath);
                ...
                Logger.Log("Access rule successfully applied.");
            }
            catch (Exception ex)
            {
                retVal = false;
