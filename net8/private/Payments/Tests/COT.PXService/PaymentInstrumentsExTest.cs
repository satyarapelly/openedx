...
namespace COT.PXService
{
    ...
    public class PaymentInstrumentsExTest : TestBase
    {
        ...
        [DataRow(null)]
        [DataRow("PXUseShortURLController")]
        [DataTestMethod]
        [TestCategory(TestCategory.INTContinuousTest)]
        public void WindowsStore_PayPalQrCode(string flight)
        {
            ...
            if (flight == "PXUseShortURLController")
            {
                string domain = "pay-ms-int.com";
                Assert.IsTrue(body.Contains(domain), "Body must contain short url with domain 'payurl-int.ms'");

                int startIndex = body.IndexOf(domain);
            }
            else
            {
