...
namespace CIT.PXService.Tests
{
    ...
    public class PaymentInstrumentsExTests : TestBase
    {
        ...
        public async Task PaymentInstrumentsEx_Kakaopay_GetPIAsExpected(string partner, string piid, string expectedDefaultDisplayName, string country)
       {
        ...
        [DataRow("commercialstores", true)]
        [DataRow("commercialstores", false)]
        [DataRow("defaulttemplate", true)]
        [DataRow("defaulttemplate", false)]
        [DataRow("setupofficesdx", true)]
        [DataRow("setupofficesdx", false)]
        [DataRow("xboxnative", true)]
        [DataRow("xboxnative", false)]
        [DataTestMethod]
        public async Task PaymentInstrumentsEx_UseTwoStaticPageRedirectionForSepa(string partner, bool isPSSEnabled)
        {
            // Arrange
            var headers = new Dictionary<string, string>() { };
            ...
            {
                paymentMethodFamily = "direct_debit",
                paymentMethodType = Constants.PaymentMethodFamilyType.Sepa,
                paymentMethodOperation = Constants.OperationTypes.Add
            };

            string postRequestUri = $"/v7.0/SepaPicvAccount/paymentInstrumentsEx?country=de&language=en-US&partner={partner}";
            PaymentInstrument piFromPims = PimsMockResponseProvider.GetPaymentInstrument("SepaPicvAccount", "SepaPicvAccount-Pi001-Redirect");
            ...
            if (isPSSEnabled)
            {
                if (partner == "azure" || partner == "azuremanage")
                {
                    var partnerSettingResponseForAzure = new
                    {
                        add = new
                        {
                            template = Constants.PartnerNames.DefaultTemplate,
                            features = new
                            {
                                useTwoStaticPageRedirection = new
                                {
                                    applicableMarkets = new string[] { },
                                    displayCustomizationDetail = (object)null
                                }
                            }
                        }
                    };

                    PXSettings.PartnerSettingsService.ArrangeResponse(JsonConvert.SerializeObject(partnerSettingResponseForAzure));
                }
                else
                {
                    var partnerSettingResponseForNonAzure = new
                    {
                        add = new
                        {
                            template = Constants.PartnerNames.DefaultTemplate,
                    ...
                    PXSettings.PartnerSettingsService.ArrangeResponse(JsonConvert.SerializeObject(partnerSettingResponseForNonAzure));
                }
            }
            ...
            // Access clientaction from clientaction
            var clientActionFromJson = json["clientAction"];
            Assert.IsNotNull(clientActionFromJson, "ClientAction should not be null");

            // Access context from clientaction
            var contextOfClientAction = clientActionFromJson["context"];
            Assert.IsNotNull(contextOfClientAction, "ClientAction.Context should not be null");

            // Access type of clientaction
            var typeOfClientAction = clientActionFromJson["type"];
            Assert.IsNotNull(typeOfClientAction, "ClientAction.Type should not be null");

            if (partner == "azure" || partner == "azuremanage")
            {
                Assert.AreEqual("Pidl", typeOfClientAction.ToString(), "ClientAction.Type should be Pidl");

                var contextItem = ((JArray)contextOfClientAction)[0];
                Assert.IsNotNull(contextItem, "Context item should not be null");

                var displayDescriptions = contextItem["displayDescription"] as JArray;
                Assert.IsNotNull(displayDescriptions, "DisplayDescription should not be null");

                Assert.AreEqual(2, displayDescriptions.Count, "There should be two display descriptions for SEPA redirection");

                Assert.IsTrue(displayDescriptions.Any(d => d["displayId"]?.ToString() == "sepaRedirectStaticPage"), "sepaRedirectStaticPage should be present");
                Assert.IsTrue(displayDescriptions.Any(d => d["displayId"]?.ToString() == "sepaStatusCheckStaticPage"), "sepaStatusCheckStaticPage should be present");

                var sepaRedirectStaticPage = displayDescriptions.First;
                var sepaStatusCheckStaticPage = displayDescriptions.Last;

                Assert.IsNotNull(sepaRedirectStaticPage, "sepaRedirectStaticPage should not be null");
                Assert.IsNotNull(sepaStatusCheckStaticPage, "sepaStatusCheckStaticPage should not be null");

                Assert.AreEqual("sepaRedirectStaticPage", sepaRedirectStaticPage["displayId"]?.ToString(), "DisplayId for sepaRedirectStaticPage should match");
                Assert.AreEqual("sepaStatusCheckStaticPage", sepaStatusCheckStaticPage["displayId"]?.ToString(), "DisplayId for sepaStatusCheckStaticPage should match");

                Assert.IsTrue(sepaRedirectStaticPage["members"] is JArray, "Members of sepaRedirectStaticPage should be a JArray");
                Assert.IsTrue(sepaStatusCheckStaticPage["members"] is JArray, "Members of sepaStatusCheckStaticPage should be a JArray");

                var sepaRedirectMembers = sepaRedirectStaticPage["members"] as JArray;
                var sepaStatusCheckMembers = sepaStatusCheckStaticPage["members"] as JArray;

                Assert.IsNotNull(sepaRedirectMembers, "Members of sepaRedirectStaticPage should not be null");
                Assert.IsNotNull(sepaStatusCheckMembers, "Members of sepaStatusCheckStaticPage should not be null");

                Assert.AreEqual(1, sepaRedirectMembers.Count, "There should be one member in sepaRedirectStaticPage");
                Assert.AreEqual(1, sepaStatusCheckMembers.Count, "There should be one member in sepaStatusCheckStaticPage");

                var sepaRedirectMember = sepaRedirectMembers[0];
                var sepaStatusCheckMember = sepaStatusCheckMembers[0];

                Assert.IsNotNull(sepaRedirectMember, "sepaRedirectMember should not be null");
                Assert.IsNotNull(sepaStatusCheckMember, "sepaStatusCheckMember should not be null");

                Assert.AreEqual("sepaRedirectButtonGroup", sepaRedirectMember["displayId"]?.ToString(), "DisplayId for sepaRedirectButtonGroup should match");
                Assert.AreEqual("sepaStatusCheckButtonGroup", sepaStatusCheckMember["displayId"]?.ToString(), "DisplayId for sepaStatusCheckButtonGroup should match");

                Assert.IsTrue(sepaRedirectMember["members"] is JArray, "Members of sepaRedirectButtonGroup should be a JArray");
                Assert.IsTrue(sepaStatusCheckMember["members"] is JArray, "Members of sepaStatusCheckButtonGroup should be a JArray");

                var sepaRedirectButtonGroupMembers = sepaRedirectMember["members"] as JArray;
                var sepaStatusCheckButtonGroupMembers = sepaStatusCheckMember["members"] as JArray;
                Assert.IsNotNull(sepaRedirectButtonGroupMembers, "Members of sepaRedirectButtonGroup should not be null");
                Assert.IsNotNull(sepaStatusCheckButtonGroupMembers, "Members of sepaStatusCheckButtonGroup should not be null");

                Assert.AreEqual(3, sepaRedirectButtonGroupMembers.Count, "There should be two members in sepaRedirectButtonGroup");
                Assert.AreEqual(2, sepaStatusCheckButtonGroupMembers.Count, "There should be two members in sepaStatusCheckButtonGroup");

                foreach (var member in sepaRedirectButtonGroupMembers)
                {
                    Assert.IsNotNull(member["displayId"]?.ToString(), "DisplayId in sepaRedirectButtonGroup member should not be null");

                    if (member["displayId"]?.ToString() == "sepaGoToBankButton")
                    {
                        Assert.IsNotNull(member["displayContent"]?.ToString(), "DisplayContent in sepaRedirectButtonGroup member should not be null");
                        Assert.IsNotNull(member["displayType"]?.ToString(), "DisplayType in sepaRedirectButtonGroup member should not be null");
                        Assert.IsNotNull(member["pidlAction"], "pidlAction in sepaRedirectButtonGroup member should not be null");
                        Assert.IsNotNull(member["tags"], "Tags in sepaRedirectButtonGroup member should not be null");
                        Assert.IsTrue(member["isHighlighted"]?.ToObject<bool?>() ?? false, "isHighlighted in sepaRedirectButtonGroup member should be true");
                        Assert.IsTrue(member["pidlAction"]["type"]?.ToString() == "navigateAndMoveNext", "pidlAction type in sepaRedirectButtonGroup member should be navigateAndMoveNext");
                        Assert.IsTrue(member["tags"]["accessibilityName"]?.ToString() == "Continue", "Accessibility name in sepaRedirectButtonGroup member should match");
                        Assert.IsTrue(member["displayContent"]?.ToString() == "Continue", "DisplayContent in sepaRedirectButtonGroup member should match");

                        var contextOfPidlAction = member["pidlAction"]["context"] as JObject;

                        var baseUrlOfContext = contextOfPidlAction["baseUrl"]?.ToString();
                        Assert.IsNotNull(baseUrlOfContext, "Redirection BaseUrl should not be null");

                        var successParamsOfContext = contextOfPidlAction["successParams"] as JObject;
                        Assert.IsNotNull(successParamsOfContext, "SuccessParams should not be null");
                        var successParamsFamily = successParamsOfContext["family"].ToString();
                        var successParamsType = successParamsOfContext["type"].ToString();
                        var successParamsPendingOn = successParamsOfContext["pendingOn"].ToString();

                        Assert.IsNotNull(successParamsFamily, "SuccessParams.family should not be null");
                        Assert.IsNotNull(successParamsType, "SuccessParams.type should not be null");
                        Assert.IsNotNull(successParamsPendingOn, "SuccessParams.pendingOn should not be null");

                        Assert.AreEqual("direct_debit", successParamsFamily);
                        Assert.AreEqual(Constants.PaymentMethodFamilyType.Sepa, successParamsType);
                        Assert.AreEqual("redirect", successParamsPendingOn);
                    }
                    else if (member["displayId"]?.ToString() == "sepaCancelButton")
                    {
                        Assert.IsNotNull(member["displayContent"]?.ToString(), "DisplayContent in sepaCancelButton should not be null");
                        Assert.IsNotNull(member["displayType"]?.ToString(), "DisplayType in sepaCancelButton should not be null");
                        Assert.IsNotNull(member["pidlAction"], "pidlAction in sepaCancelButton should not be null");
                        Assert.IsNotNull(member["tags"], "Tags in sepaCancelButton should not be null");
                        Assert.IsFalse(member["isHighlighted"]?.ToObject<bool?>() ?? false, "isHighlighted in sepaCancelButton should be false");
                        Assert.IsTrue(member["pidlAction"]["type"]?.ToString() == "gohome", "pidlAction type in sepaCancelButton should be gohome");
                        Assert.IsTrue(member["tags"]["accessibilityName"]?.ToString() == "Cancel", "Accessibility name in sepaCancelButton should match");
                        Assert.IsTrue(member["displayContent"]?.ToString() == "Cancel", "DisplayContent in sepaCancelButton should match");
                    }
                    else if (member["displayId"]?.ToString() == "sepaRedirectText")
                    {
                        Assert.IsNotNull(member["displayContent"]?.ToString(), "DisplayContent in sepaRedirectText should not be null");
                        Assert.IsNotNull(member["displayType"]?.ToString(), "DisplayType in sepaRedirectText should not be null");
                        Assert.IsNotNull(member["tags"], "Tags in sepaRedirectText should not be null");
                        Assert.IsTrue(member["tags"]["accessibilityName"]?.ToString() == "You will be redirected to complete verification of this payment method. Please Turn off the PopUp Blocker", "Accessibility name in sepaRedirectText should match");
                        Assert.IsTrue(member["displayContent"]?.ToString() == "You will be redirected to complete verification of this payment method. Please Turn off the PopUp Blocker", "DisplayContent in sepaRedirectText should match");
                    }
                }

                foreach (var member in sepaStatusCheckButtonGroupMembers)
                {
                    Assert.IsNotNull(member["displayId"]?.ToString(), "DisplayId in sepaStatusCheckButtonGroup member should not be null");

                    if (member["displayId"]?.ToString() == "sepaStatusCheckYesButton")
                    {
                        Assert.IsNotNull(member["displayContent"]?.ToString(), "DisplayContent in sepaStatusCheckYesButton should not be null");
                        Assert.IsNotNull(member["displayType"]?.ToString(), "DisplayType in sepaStatusCheckYesButton should not be null");
                        Assert.IsNotNull(member["pidlAction"], "pidlAction in sepaStatusCheckYesButton should not be null");
                        Assert.IsNotNull(member["tags"], "Tags in sepaStatusCheckYesButton should not be null");
                        Assert.IsTrue(member["isHighlighted"]?.ToObject<bool?>() ?? false, "isHighlighted in sepaStatusCheckYesButton should be true");
                        Assert.IsTrue(member["pidlAction"]["type"]?.ToString() == "submit", "pidlAction type in sepaStatusCheckYesButton should be submit");
                        Assert.IsTrue(member["tags"]["accessibilityName"]?.ToString() == "Yes, I am done with the verification", "Accessibility name in sepaStatusCheckYesButton should match");
                        Assert.IsTrue(member["displayContent"]?.ToString() == "Yes, I am done with the verification", "DisplayContent in sepaStatusCheckYesButton should match");

                        var contextOfPidlAction = member["pidlAction"]["context"] as JObject;

                        var hrefOfContext = contextOfPidlAction["href"]?.ToString();
                        Assert.IsNotNull(hrefOfContext, "StatusCheck href should not be null");

                        var methodOfHref = contextOfPidlAction["method"]?.ToString();
                        Assert.IsNotNull(methodOfHref, "StatusCheck method should not be null");

                        Assert.AreEqual("https://{pifd-endpoint}/users/{userId}/paymentInstrumentsEx/SepaPicvAccount-Pi001-Redirect?language=en-US&partner=" + partner + "&country=de", hrefOfContext, "Href in pidlAction should match the baseUrl in context");
                        Assert.AreEqual("GET", methodOfHref, "Method in pidlAction should match the method in context");
                    }
                    else if (member["displayId"]?.ToString() == "sepaCancelButton")
                    {
                        Assert.IsNotNull(member["displayContent"]?.ToString(), "DisplayContent in sepaCancelButton should not be null");
            ...
            {
                Assert.AreEqual("Redirect", typeOfClientAction.ToString(), "ClientAction.Type should be Redirect");

                var baseUrlOfContext = contextOfClientAction["baseUrl"]?.ToString();
                Assert.IsNotNull(baseUrlOfContext, "Redirection BaseUrl should not be null");

                var successParamsOfContext = contextOfClientAction["successParams"] as JObject;
                Assert.IsNotNull(successParamsOfContext, "SuccessParams should not be null");
                var successParamsId = successParamsOfContext["id"].ToString();
                var successParamsFamily = successParamsOfContext["family"].ToString();
                var successParamsType = successParamsOfContext["type"].ToString();
                var successParamsPendingOn = successParamsOfContext["pendingOn"].ToString();
                var successParamsPicvRequired = successParamsOfContext["picvRequired"].ToString();

                Assert.IsNotNull(successParamsId, "SuccessParams.id should not be null");
                Assert.IsNotNull(successParamsFamily, "SuccessParams.family should not be null");
                Assert.IsNotNull(successParamsType, "SuccessParams.type should not be null");
                Assert.IsNotNull(successParamsPendingOn, "SuccessParams.pendingOn should not be null");
                Assert.IsNotNull(successParamsPicvRequired, "SuccessParams.picvRequired should not be null");

                Assert.AreEqual("SepaPicvAccount-Pi001-Redirect", successParamsId);
                Assert.AreEqual("direct_debit", successParamsFamily);
                Assert.AreEqual(Constants.PaymentMethodFamilyType.Sepa, successParamsType);
                Assert.AreEqual("redirect", successParamsPendingOn);
                Assert.AreEqual("False", successParamsPicvRequired);

                var displayDescriptions = contextOfClientAction["displayDescription"] as JArray;
                Assert.IsNull(displayDescriptions, "DisplayDescription should be null");
            }
