...
namespace CIT.PXService.Tests
{
    ...
    public class RewardsDescriptionsControllerTests : TestBase
    {
        ...
        [DataRow("US", "en-US", "storify", 10, "usd", "00001", 2000, true, true)]
        [DataRow("US", "en-US", "storify", 10, "usd", "00001", 2000, true, false)]
        [DataRow("US", "en-US", "storify", 10, "usd", "00001", 2000, false, true)]
        [DataRow("US", "en-US", "storify", 10, "usd", "00001", 2000, false, false)]
        [TestMethod]
        public async Task RedeemMSRewardsXboxNative(string country, string language, string partner, int orderAmount, string currency, string catalogSku, long rewardsPoints, bool isVariableAmountSku, bool styleHintsFlight)
        {
            // Arrange
            string userPuid = "844426234689219";
            string deviceId = "1234567890";
            ...
            string rewardsContext = JsonConvert.SerializeObject(rewardsContextData);
            string flights = "PXEnableXboxNativeRewards";
            if (styleHintsFlight)
            {
            ...
            foreach (var pidl in pidls)
            {
                ...
                if (styleHintsFlight)
                {
                    Assert.IsTrue(redeemMSRewardsPage.StyleHints.Count > 0);
                    Assert.IsTrue(redeemMSRewardsPage.StyleHints.SequenceEqual(new List<string> { "height-fill" }));
                }
                else
                {
                    Assert.IsNull(redeemMSRewardsPage.StyleHints);
                }
