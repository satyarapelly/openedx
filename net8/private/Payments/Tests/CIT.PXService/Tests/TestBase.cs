...
namespace CIT.PXService.Tests
{
    ...
    public class TestBase
    {
        ...
        public static async Task<List<PIDLResource>> GetPidlFromPXServiceWithPartnerHeader(string url, string headerKey, string headerValue, string expectedRequestHeaderValue)
        {
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, GetPXServiceUrl(url));
            ...
            // Assert what the request *currently* carries (before send)
            if (string.IsNullOrEmpty(expectedRequestHeaderValue))
            {
                // Hack: hard-remove x-ms-flight explicitly for this request
                request.Headers.Remove("x-ms-flight");

                Assert.IsFalse(request.Headers.TryGetValues(headerKey, out _),
                    $"Request should not contain header '{headerKey}'.");
            }
            else
            {
                Assert.IsTrue(request.Headers.TryGetValues(headerKey, out var preValues),
                    $"Request should contain header '{headerKey}'.");
                var preValue = preValues.SingleOrDefault();
                Assert.AreEqual(expectedRequestHeaderValue, preValue,
                    $"Unexpected value for header '{headerKey}'.");
            }
