...
namespace Microsoft.Commerce.Payments.Common.OpenTelemetry
{
    ...
    public static class SllLogger
    {
            ...
            CommerceServiceRequestStatus serviceRequestStatus)
        {
            ...
            if (succeeded)
            {
                logger.LogInformation(SllLoggerMessageFormat, requestTraceId, serverTraceId, serviceName, "Microsoft.Commerce.Tracing.Sll.ClientOperationDetails", message, JsonConvert.SerializeObject(operationDetails));
            }
            else
            {
                logger.LogError(SllLoggerMessageFormat, requestTraceId, serverTraceId, serviceName, "Microsoft.Commerce.Tracing.Sll.ClientOperationDetails", message, JsonConvert.SerializeObject(operationDetails));
            }
        }
    }
}
