...
namespace Microsoft.Commerce.Payments.PidlFactory.V7
{
    ...
    public sealed class PIDLResourceFactory
    {
        ...
        public List<PIDLResource> GetSepaRedirectAndStatusCheckDescriptionForPI(
            PaymentInstrument paymentInstrument,
            string language,
            string partnerName,
            string scenario,
            ...
            PaymentExperienceSetting setting = null)
        {
            List<PIDLResource> retList = this.GetStaticPidlDescriptions(Constants.StaticDescriptionTypes.SepaRedirectAndStatusCheckPidl, language, partnerName, setting: setting);
            ...
            foreach (var pidlResource in retList)
            {
                var goToBankButton = PIDLResourceDisplayHintFactory.Instance.GetDisplayHintById<ButtonDisplayHint>(Constants.ButtonDisplayHintIds.SepaGoToBankButton, pidlResource.DisplayPages);
                if (goToBankButton != null)
                {
                    goToBankButton.Action.Context = redirectionLink;
                }
        ...
        public List<PIDLResource> GetRewardsDescriptions(string rewardsType, string operation, string country, string language, string partnerName = Constants.PidlConfig.DefaultPartnerName, List<string> exposedFlightFeatures = null)
        {
            ...
            // PIDL page for "redeem-GET" operation of xpay is empty and not made from CSVs
            if (!string.Equals(Constants.PidlOperationTypes.Redeem, operation, StringComparison.InvariantCultureIgnoreCase) ||
                (exposedFlightFeatures?.Contains(Flighting.Features.PXEnableXboxNativeRewards, StringComparer.OrdinalIgnoreCase) ?? false))
            {
        ...
        private static Dictionary<string, RestLink> GetDeviceBindingSubmitLink(string ntid, string challengeId, string partnerName, string country, string language, string challengeMethodId)
        {
            ...
            retLinks[Constants.ButtonDisplayHintIds.VerifySubmitButton] = submitUrlLink;
        ...
        private static Dictionary<string, RestLink> GetPILink(string restResourceName, string language, string partnerName, string piid, bool completePrerequisites, string country, string scenario = null, string sessionQueryUrl = null, string classicProduct = null)
        {
            ...
            retLinks[Constants.ButtonDisplayHintIds.SepaStatusCheckYesButton] = submitUrlLink;
