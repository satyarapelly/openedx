...
namespace Microsoft.Commerce.Payments.PXService
{
    ...
    public class PurchaseServiceAccessor : IPurchaseServiceAccessor
    {
        ...
        public static class URIValidate
        {
            /// <summary>
            /// Validates if the given URL is within the expected domain.
            /// </summary>
            /// <param name="url">The full URL to validate (e.g., nextLink).</param>
            /// <param name="expectedHost">The trusted base host/domain (e.g., "api.mycompany.com").</param>
            /// <returns>True if the URL belongs to the expected host; false otherwise.</returns>
            public static bool InDomain(string url, string expectedHost)
            {
                if (string.IsNullOrWhiteSpace(url) || string.IsNullOrWhiteSpace(expectedHost))
                {
                    return false;
                }
