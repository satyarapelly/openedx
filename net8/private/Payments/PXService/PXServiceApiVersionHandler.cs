...
namespace Microsoft.Commerce.Payments.PXService
{
    ...
    public class PXServiceApiVersionHandler
    {
        ...
        public async Task InvokeAsync(HttpContext httpContext)
        {
            ...
            using var request = httpContext.Request.ToHttpRequestMessage();
            request.Content = new StreamContent(httpContext.Request.Body);

            // Mirror old SetProperty onto both request message and HttpRequest
            ...
            // Extract version: {version} route value OR api-version header OR URL segment fallback
            string externalVersion =
                (routeValues.TryGetValue("version", out var vObj) ? Convert.ToString(vObj) : null)
                ?? httpContext.Request.Headers["api-version"].ToString();

            if (string.IsNullOrWhiteSpace(externalVersion))
            {
                httpContext.Response.StatusCode = StatusCodes.Status400BadRequest;
                await httpContext.Response.WriteAsync("The request must contain an api-version header.");
                return;
            }

            if (!this.supportedVersions.TryGetValue(externalVersion, out var apiVersion))
            {
                httpContext.Response.StatusCode = StatusCodes.Status400BadRequest;
                await httpContext.Response.WriteAsync($"Unsupported api-version '{externalVersion}'.");
                return;
            }
            ...
            // Fallback: from route values (works for conventional routes)
            controllerName = httpContext.Request.RouteValues.TryGetValue("controller", out var controller)
                ? Convert.ToString(controller)
                : null;
            ...
            if (string.IsNullOrWhiteSpace(externalVersion))
            {
                ...
                await httpContext.Response.WriteAsync(
                    "{\"error\":\"The request must contain an api-version header.\"}");
                return;
            }
            ...
            if (!this.supportedVersions.TryGetValue(externalVersion, out apiVersion))
            {
                ...
                httpContext.Response.ContentType = "application/json";
                await httpContext.Response.WriteAsync(
                    $"{{\"error\":\"Unsupported api-version '{externalVersion}'.\"}}");
                return;
            }

            // make the chosen version available to later components
            httpContext.Items[PaymentConstants.Web.Properties.Version] = apiVersion;
            ...
            // Add response headers just before Core writes the response
            httpContext.Response.OnStarting(() =>
            {
                ...
                if (exposableFeatures?.Contains(Flighting.Features.PXSendContentTypeOptionsHeader, StringComparer.OrdinalIgnoreCase) == true)
                    httpContext.Response.Headers[ContentTypeOptionsHeader] = NoSniff;

                if (exposableFeatures?.Contains(Flighting.Features.PXSendNoRetryOnServerErrorHeader, StringComparer.OrdinalIgnoreCase) == true)
                    httpContext.Response.Headers[RetryOnServerErrorHeader] = "false";

                return Task.CompletedTask;
            });

            // Rewind the body so model binding sees it from the start
