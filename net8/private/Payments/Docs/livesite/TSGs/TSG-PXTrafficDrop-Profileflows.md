## PX Traffic Drop - Profile flows 

__PX Traffic Drop - Profile flows__

* __Background__:  This TSG is for investigating the traffic drop of Profile flows operation name .The issue involves a significant drop in traffic across profile flows, impacting the PX service.

* __Issue__: After get an IcM generated by PX Traffic Drop - Profile flows monitors, always start your investigation from IcM itself. Use [Incident 599633323](https://portal.microsofticm.com/imp/v5/incidents/details/599633323/summary)  Details - IcM  as an sample.

* __Impact__: We've noticed that a particular operation name :  ProfileDescriptionsController-GET having traffic Drop.
 ...
 * __Steps to Investigate__:
    ...
    3. __Modify Operation Name & Time__: Adjust the operation name and time based on the impacted operations and time window.
          * ProfileDescriptionsController-GET

* __Sample Query__ 
  ```
  cluster("pst").database("Prod").RequestTelemetry 
  | where name == "Microsoft.Commerce.Tracing.Sll.PXServiceIncomingOperation" 
  | where data_baseData_operationName contains "ProfileDescriptionsController-GET"//"PaymentMethodDescriptionsController-GET" 
  | where TIMESTAMP between (datetime(2025-02-23 06:01:06Z) .. datetime(2025-02-23 08:31:06Z)) 
...
*  __Run the below query to check the system errors and user errors__
    ...
    ```
    let startTime = datetime(2025-02-23 06:01:06Z);  //ICM Troubleshooting Information
    let endTime = datetime(2025-02-23 08:31:06Z);
    let operationName = "ProfileDescriptionsController-GET";
...
* __Run the below query to check the exact error.__
    ...
    | where data_baseData_operationName == "ProfileDescriptionsController-GET"
...
* __Root Cause Analysis__:
  ...
  Check if the monitor for the operation name: ProfileDescriptionsController-GET is back to healthy or if it is continuously dropping. Then, work with the PaymentExperience team to identify the root cause and update the Root Cause Analysis in the IcM.
  
* __Verification Steps__:

  If the issue is auto-mitigated, then we have to check the [Monitors | Jarvis](https://portal.microsoftgeneva.com/manage/monitors-direct?activity=monitor-home&account=paymentexperience-metrics-prod) attached, go to the Monitor and confirm if the operation name: ProfileDescriptionsController-GET is back to healthy. If not, Kindly contact to PaymentExperience team.
