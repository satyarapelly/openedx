## PX Traffic Drop - 3DS1 flow
__PX Traffic Drop - 3DS1 flow__

* __Background__: This TSG is for investigating the traffic drop of 3DS1 flows operation names .The issue involves a significant drop in traffic across 3DS1 flows, impacting the PX service.

* __Issue__: After get an IcM generated by PX Traffic Drop - 3DS1 flows monitors, always start your investigation from IcM itself. Use [Incident 593597716](https://portal.microsofticm.com/imp/v5/incidents/details/593597716/summary)  Details - IcM  as an sample.

* __Impact__: We've noticed that a particular operation name : PaymentSessionsController-GET-GetPaymentSession having traffic Drop.
...
* __Steps to Investigate__:
    ...
    3. __Modify Operation Name & Time__: Adjust the operation name and time based on the impacted operations and time window.
          * PaymentSessionsController-POST-BrowserAuthenticateThreeDSOne-S
          * PaymentSessionsController-GET-GetPaymentSession
          * PaymentSessionsController-GET-BrowserAuthenticateR3DS1-S
          * PaymentSessionsController-POST-BrowserN3DS1ChallengeCompleted-S

* __Sample Query__ 
    ```
    cluster("pst").database("Prod").RequestTelemetry 
    | where name == "Microsoft.Commerce.Tracing.Sll.PXServiceIncomingOperation" 
    | where data_baseData_operationName contains "PaymentSessionsController-GET-GetPaymentSession"//"PaymentSessionsController-POST-BrowserN3DS1ChallengeCompleted-L"
...
*  __Run the below query to check the system errors and user errors__
    ...
    let operationName = "PaymentSessionsController-GET-GetPaymentSession";
...
*  * __Run the below query to check the exact error.__
    ...
    | where data_baseData_operationName == "PaymentSessionsController-GET-GetPaymentSession"
...
* __Root Cause Analysis__:
  ...
  Check if the monitor for the operation name:  PaymentSessionsController-GET-GetPaymentSession is back to healthy or if it is continuously dropping. Then, work with the PaymentExperience team to identify the root cause and update the Root Cause Analysis in the IcM.
  
* __Verification Steps__:

  If the issue is auto-mitigated, then we have to check the [Monitors | Jarvis](https://portal.microsoftgeneva.com/manage/monitors-direct?activity=monitor-home&account=paymentexperience-metrics-prod) attached, go to the Monitor and confirm if the operation name:  PaymentSessionsController-GET-GetPaymentSession is back to healthy. If not, Kindly contact to PaymentExperience team.
