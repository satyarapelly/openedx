...
namespace Microsoft.Commerce.Payments.Common.OpenTelemetry
{
    ...
    public static class SllWebLogger
    {
            ...
            string message)
        {
            ...
            if (response != null)
            {
                if (response.IsSuccessStatusCode || response.StatusCode == HttpStatusCode.SeeOther)
                {
            ...
            string cv = request.GetCorrelationVector() == null ? string.Empty : request.GetCorrelationVector().ToString();

            if (succeeded)
            {
                logger.LogInformation(SllWebLoggerLogMessageFormat, requestTraceId, serverTraceId, cv, request.GetServiceName(), string.Empty, "Microsoft.Commerce.Tracing.Sll.OperationDetails", message, JsonConvert.SerializeObject(operationDetails));
            }
            else
            {
                logger.LogError(SllWebLoggerLogMessageFormat, requestTraceId, serverTraceId, cv, request.GetServiceName(), string.Empty, "Microsoft.Commerce.Tracing.Sll.OperationDetails", message, JsonConvert.SerializeObject(operationDetails));
            }
        }
          ...
          string avsSuggest = null)
        {
            ...
            if (succeeded)
            {
                logger.LogInformation(SllWebLoggerLogMessageFormat, requestTraceId, serverTraceId, cv, request.GetServiceName(), string.Empty, "Microsoft.Commerce.Tracing.Sll.PXServiceIncomingOperation", message, JsonConvert.SerializeObject(incomingOperation));
            }
            else
            {
                logger.LogError(SllWebLoggerLogMessageFormat, requestTraceId, serverTraceId, cv, request.GetServiceName(), string.Empty, "Microsoft.Commerce.Tracing.Sll.PXServiceIncomingOperation", message, JsonConvert.SerializeObject(incomingOperation));
            }
        }
           ...
           string servicePointData = null)
        {
            ...
            var operationDetails = new
            {
                ...
                baseData = new
                {
                    ...
                    dependencyType = string.Empty, // Task 1562648:[PIMS TODOs] ClientOperation SllLogger enhancement. Figure out how to make the best use of this field.
            ...
            {
                correlationVector = request.GetCorrelationVector() == null ? string.Empty : request.GetCorrelationVector().ToString();
            }
           ...
           string puid)
        {
            ...
            logger.LogInformation(SllWebLoggerLogMessageFormat, requestTraceId, string.Empty, correlationVector, request.GetServiceName(), string.Empty, "Microsoft.Commerce.Tracing.Sll.PXServicePIAddedOnOffer", string.Empty, JsonConvert.SerializeObject(paymentInstrumentAddedOnOffer));
        }
