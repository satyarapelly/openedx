...
namespace Microsoft.Commerce.Payments.PXService
{
    ...
    public static class HttpRequestHelper
    {
        ...
        {
            request.Headers.TryGetValues(GlobalConstants.HeaderValues.PidlSdkVersion, out IEnumerable<string>? xboxNativePidlsdkVersions);

            if (xboxNativePidlsdkVersions != null)
            {
                List<string> pidlsdkVersionDetails = xboxNativePidlsdkVersions.FirstOrDefault()?.Split('_').ToList() ?? new List<string>();
                ...
                if (pidlsdkVersionDetails.Count > 0)
                {
                    if (pidlsdkVersionDetails[0].Contains(V7.Constants.Versions.Alpha))
                    {
                        ...
                        Version.TryParse(pidlsdkAlphaVersionExists[0] + "." + pidlsdkAlphaVersionDetails[1], out xboxNativePidlSdkVersionConcat);
                        return xboxNativePidlSdkVersionConcat;
                    }
                    else
                    {
                        Version.TryParse(pidlsdkVersionDetails[0] + ".0", out xboxNativePidlSdkVersionConcat);
                        return xboxNativePidlSdkVersionConcat;
                    }
                }
            }
        ...
        public static void SetTestHeader(string value)
        {
            ...
            if (headers != null)
            {
                headers[PaymentConstants.PaymentExtendedHttpHeaders.TestHeader] = value;
            }
        }
