        ...
        public override async Task<List<PIDLResource>> GetDescription()
        {
            ...
            else if (clientAction.Type == ClientActionType.HandleChallenge && clientAction.ChallengeType == PaymentInstrumentChallengeType.ThreeDs2)
            {
                ...
                PIDLResource clientActionFor3DS2 = await this.generate3DS2ChallengePIDLResource(
                    GetPaymentSessionData(this.Partner ?? TemplateName.DefaultTemplate, this.PaymentRequestClientActions, challengeWindowSize: this.ChallengeWindowSize, setting: this.PSSSetting),
                    this.requestContext,
                    this.activityId,
                    this.PSSSetting);
                ...
                {
                    SllWebLogger.TraceServerMessage("GetDescription", this.activityId.CorrelationVectorV4.Value, this.activityId.ActivityId.ToString(), "ThreeDs2 challenge is required but clientActionFor3DS2 is null", Diagnostics.Tracing.EventLevel.Warning);
                    return GetClientActionResource(this.PaymentRequestClientActions);
                }
            }
            else
