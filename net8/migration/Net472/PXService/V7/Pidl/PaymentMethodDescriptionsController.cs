...
namespace Microsoft.Commerce.Payments.PXService.V7
{
    ...
    public class PaymentMethodDescriptionsController : ProxyController
    {
            ...
            string sessionId = null)
        {
            ...
            PaymentsEventSource.Log.InstrumentManagementServiceTraceRequest(apiName, this.Request.RequestUri.AbsolutePath, traceActivityId);
            ...
            string sessionId = null)
        {
            ...
            {
                PaymentsEventSource.Log.PXServiceTraceRequest(apiName, this.Request.RequestUri.AbsolutePath, traceActivityId);
            }
            else
            {
                PaymentsEventSource.Log.InstrumentManagementServiceTraceRequest(apiName, this.Request.RequestUri.AbsolutePath, traceActivityId);
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(operation))
                {
                        ...
                        operation.Equals(Constants.Operations.ExpressCheckout, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        {
                            PaymentsEventSource.Log.PXServiceMissingRiskParameter(orderId, sessionId, actualPuid, traceActivityId);
                        }
                        ...
                        if (operation.Equals(Constants.Operations.ExpressCheckout, StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            await expressCheckout.LoadComponentDescription(null, this.Settings, traceActivityId, setting, this.ExposedFlightFeatures, operation: operation, partner: partner, country: country, language: expressCheckout.ExpressCheckoutRequest?.Language, currency: currency, scenario: scenario, request: this.Request);
                    ...
                    && string.Equals(Request?.GetQueryNameValuePairs()?.FirstOrDefault(i => i.Key == Constants.QueryParameterName.ShowChallenge).Value, bool.TrueString, StringComparison.OrdinalIgnoreCase))
                {
                    if (!string.IsNullOrEmpty(pxChallengeSessionId))
                    {
                        ...
                        catch (ServiceErrorResponseException serviceError)
                        {
                            ...
                            {
                                PaymentsEventSource.Log.TracingHandlerTraceError($"Conflict Error received with while adding challenge with message: {serviceError.Message}. Marking SessionId : {pxChallengeSessionId} as Abandoned", traceActivityId);
                                await PXChallengeManagementHandler.UpdatePXSessionAbandonedStatus(pxChallengeSessionId, traceActivityId);
                            }
                            else
                            {
                                PaymentsEventSource.Log.TracingHandlerTraceError($"Error occured while adding challenge with message: {serviceError.Message}.", traceActivityId);
                            }
                        }
                        catch (Exception ex)
                        {
                            PaymentsEventSource.Log.TracingHandlerTraceError($"Error occured while adding challenge with message: {ex.Message}.", traceActivityId);
                        }
                    }
                }
            }
