    ...
    public class PaymentSessionsHandlerV2 : PaymentSessionsHandler
    {
            ...
            TestContext testContext)
        {
            ...
            try
            {
                ...
                SllWebLogger.TraceServerMessage("Authenticate App", null, sessionId, payerAuthResponse?.TransactionStatus != null ? "TransStatus: " + payerAuthResponse.TransactionStatus.ToString() : "No transStatus available", Diagnostics.Tracing.EventLevel.Informational);
                ...
                // TODO : Remove after Fraud Investigation
                SllWebLogger.TraceServerMessage("Authenticate App", null, sessionId, "Mapped Status1: " + mappedStatus.ToString(), Diagnostics.Tracing.EventLevel.Informational);
                ...
                // TODO : Remove after Fraud Investigation
                SllWebLogger.TraceServerMessage("Authenticate App", null, sessionId, "Mapped Status2: " + mappedStatus.ToString(), Diagnostics.Tracing.EventLevel.Informational);

                authenticationVerified = PaymentSessionsHandler.IsAuthenticationVerified(mappedStatus);

                // TODO : Remove after Fraud Investigation
                SllWebLogger.TraceServerMessage("Authenticate App", null, sessionId, "AuthenticationVerified: " + authenticationVerified.ToString(), Diagnostics.Tracing.EventLevel.Informational);
            ...
            catch (Exception ex)
            {
                ...
                // TODO : Remove after Fraud Investigation
                SllWebLogger.TraceServerMessage("Authenticate App", null, sessionId, this.storedSession != null ? "Failure, storedSession: " + JsonConvert.SerializeObject(this.storedSession) : "No stored session", Diagnostics.Tracing.EventLevel.Informational);
            ...
            {
                // TODO : Remove after Fraud Investigation
                SllWebLogger.TraceServerMessage("Authenticate App", null, sessionId, this.storedSession != null ? "storedSession: " + JsonConvert.SerializeObject(this.storedSession) : "No stored session", Diagnostics.Tracing.EventLevel.Informational);
            ...
            Action errorOperation = null)
        {
            ...
            catch (ServiceErrorResponseException ex)
            {
                ...
                {
                    SllWebLogger.TracePXServiceException($"SafetyNet caught ServiceErrorResponseException: {ex}, {ex.Error}, {ex.Error?.ErrorCode}, {ex.Response?.StatusCode.ToString()}", traceActivity);
                    return true;
                }
                else
                {
                    ...
                    {
                        SllWebLogger.TracePXServiceException($"ServiceErrorResponseException, No error handling flight, executing errorOperation : {ex}, {ex.Error}, {ex.Error?.ErrorCode}, {ex.Response?.StatusCode}", traceActivity);
                        errorOperation();
                    }
                    else
                    {
                        SllWebLogger.TracePXServiceException($"Error Operation caught ServiceErrorResponseException:: {ex}, {ex.Error}, {ex.Error?.ErrorCode}, {ex.Response?.StatusCode}", traceActivity);
                        throw;
                    }
                }
            }
            catch (Exception ex)
            {
                if (exposedFeatures != null && exposedFeatures.Contains(errorHandlingFlightName) && errorOperation != null)
                {
                    SllWebLogger.TracePXServiceException($"nonServiceErrorResponseException, Containing error handling umbrella flight, executing errorOperation : {ex}, {ex.Message}", traceActivity);
                    errorOperation();
                }
                else
                {
        ...
        private async Task UpdateSessionOnFinally(string sessionId, string accountId, bool authenticationVerified, EventTraceActivity traceActivityId)
        {
            ...
            // TODO : Remove after Fraud Investigation
            SllWebLogger.TraceServerMessage("UpdateSessionOnFinally", null, sessionId, this.storedSession != null ? "storedSession: " + JsonConvert.SerializeObject(this.storedSession) : "No stored session", Diagnostics.Tracing.EventLevel.Informational);
            ...
            IEnumerable<KeyValuePair<string, string>> piQueryParams = null)
        {
            ...
            try
            {
                ...
                // TODO : Remove after Fraud Investigation
                SllWebLogger.TraceServerMessage("Authenticate Browser", null, paymentSessionId, authResponse?.TransactionStatus != null ? "TransStatus: " + authResponse.TransactionStatus.ToString() : "No transStatus available", Diagnostics.Tracing.EventLevel.Informational);
                ...
                // TODO : Remove after Fraud Investigation
                SllWebLogger.TraceServerMessage("Authenticate Browser", null, paymentSessionId, "MappedStatus: " + mappedStatus.ToString(), Diagnostics.Tracing.EventLevel.Informational);
                ...
                if (mappedStatus != PaymentChallengeStatus.Unknown)
                {
                    ...
                    // TODO : Remove after Fraud Investigation
                    SllWebLogger.TraceServerMessage("Authenticate Browser", null, paymentSessionId, "AuthenticationVerified: " + authenticationVerified.ToString(), Diagnostics.Tracing.EventLevel.Informational);
            ...
            {
                // TODO : Remove after Fraud Investigation
                SllWebLogger.TraceServerMessage("Authenticate Browser", null, paymentSessionId, "AuthenticationVerified In Finally: " + authenticationVerified, Diagnostics.Tracing.EventLevel.Informational);
