...
namespace Microsoft.Commerce.Payments.PXService.V7.PaymentChallenge
{
    ...
    public class PaymentSessionsController : ProxyController
    {
        ...
        public async Task<PaymentSession> Post([FromUri]string accountId)
        {
            ...
            PaymentsEventSource.Log.InstrumentManagementServiceTraceRequest(PXService.GlobalConstants.APINames.PostPaymentSession, this.Request.RequestUri.AbsolutePath, traceActivityId);
        ...
        {
            EventTraceActivity traceActivityId = this.Request.GetRequestCorrelationId();
            PaymentsEventSource.Log.InstrumentManagementServiceTraceRequest(GlobalConstants.APINames.GetPaymentSession, this.Request.RequestUri.AbsolutePath, traceActivityId);
        ...
        {
            EventTraceActivity traceActivityId = this.Request.GetRequestCorrelationId();
            PaymentsEventSource.Log.InstrumentManagementServiceTraceRequest(GlobalConstants.APINames.Authenticate, this.Request.RequestUri.AbsolutePath, traceActivityId);
        ...
        {
            EventTraceActivity traceActivityId = this.Request.GetRequestCorrelationId();
            PaymentsEventSource.Log.InstrumentManagementServiceTraceRequest(GlobalConstants.APINames.CreateAndAuthenticate, this.Request.RequestUri.AbsolutePath, traceActivityId);
            string payload = await this.Request.GetRequestPayload();
        ...
        {
            EventTraceActivity traceActivityId = this.Request.GetRequestCorrelationId();
            PaymentsEventSource.Log.InstrumentManagementServiceTraceRequest(GlobalConstants.APINames.NotifyThreeDSChallengeCompleted, this.Request.RequestUri.AbsolutePath, traceActivityId);
        ...
        public async Task<HttpResponseMessage> Authenticate([FromUri] string sessionId)
        {
            ...
            {
                EventTraceActivity traceActivityId = this.Request.GetRequestCorrelationId();
                PaymentsEventSource.Log.InstrumentManagementServiceTraceRequest(GlobalConstants.APINames.NotifyThreeDSMethodCompleted, this.Request.RequestUri.AbsolutePath, traceActivityId);
        ...
        {
            EventTraceActivity traceActivityId = this.Request.GetRequestCorrelationId();
            PaymentsEventSource.Log.InstrumentManagementServiceTraceRequest(GlobalConstants.APINames.BrowserAuthenticateThreeDSOne, this.Request.RequestUri.AbsolutePath, traceActivityId);
        ...
        {
            EventTraceActivity traceActivityId = this.Request.GetRequestCorrelationId();
            PaymentsEventSource.Log.InstrumentManagementServiceTraceRequest(GlobalConstants.APINames.BrowserAuthenticateRedirectionThreeDSOne, this.Request.RequestUri.AbsolutePath, traceActivityId);
        ...
        public async Task<HttpResponseMessage> BrowserNotifyThreeDSOneChallengeCompleted([FromUri] string sessionId)
        {
            ...
            {
                EventTraceActivity traceActivityId = this.Request.GetRequestCorrelationId();
                PaymentsEventSource.Log.InstrumentManagementServiceTraceRequest(GlobalConstants.APINames.BrowserNotifyThreeDSOneChallengeCompleted, this.Request.RequestUri.AbsolutePath, traceActivityId);
        ...
        {
            EventTraceActivity traceActivityId = this.Request.GetRequestCorrelationId();
            PaymentsEventSource.Log.InstrumentManagementServiceTraceRequest(GlobalConstants.APINames.AuthenticateIndiaThreeDS, this.Request.RequestUri.AbsolutePath, traceActivityId);
        ...
        {
            // Remove Logging: https://microsoft.visualstudio.com/OSGS/_workitems/edit/58489369
            if (this.ExposedFlightFeatures != null && this.ExposedFlightFeatures.Contains(Flighting.Features.ValidatePaymentSessionPropertiesLogging, StringComparer.OrdinalIgnoreCase))
            {
                var authStatusParams = new
                {
                    Uri = this.Request?.RequestUri?.ToString(),
                    paymentContext,
                };

                SllWebLogger.TraceServerMessage("VerifyAdditionalSessionDataParamsAnomaly_1", piId, sessionId, JsonConvert.SerializeObject(authStatusParams), Diagnostics.Tracing.EventLevel.Informational);
            }

            EventTraceActivity traceActivityId = this.Request.GetRequestCorrelationId();
            PaymentsEventSource.Log.InstrumentManagementServiceTraceRequest(GlobalConstants.APINames.AuthenticationStatus, this.Request.RequestUri.AbsolutePath, traceActivityId);
            ...
            try
            {
                ...
                PXService.Model.PXInternal.PaymentSession paymentSession = null;

                // Flip the order of the getStoredSession parameters to prioritize sessionId over piSession.SessionId, 
                if (this.ExposedFlightFeatures != null && this.ExposedFlightFeatures.Contains(Flighting.Features.PXEnableGetSessionWithSessionId, StringComparer.OrdinalIgnoreCase)) 
                {
                    // Clean up flight when complete: https://microsoft.visualstudio.com/OSGS/_workitems/edit/58419823
                    paymentSession = await PaymentSessionsHandler.TryGetStoredSession(sessionId, traceActivityId);

                    if (paymentSession == null)
                    {
                        paymentSession = await PaymentSessionsHandler.TryGetStoredSession(piSession?.SessionId, traceActivityId);
                    }
                }
                else
                {
                ...
                if (paymentSession != null)
                {
                            ...
                            || this.ExposedFlightFeatures.Contains(Flighting.Features.ValidatePaymentSessionPropertiesLogging, StringComparer.OrdinalIgnoreCase)))
                    {
                        if (paymentContext != null)
                        {
                            ...
                            SllWebLogger.TraceServerMessage("VerifyAdditionalSessionDataParamsAnomaly_2", piId, sessionId, JsonConvert.SerializeObject(authStatusParams), Diagnostics.Tracing.EventLevel.Informational);

                            verficationResult = VerifyAdditionalSessionDataParams(paymentSession, paymentContext, piId, this.ExposedFlightFeatures, requestUri: this.Request.RequestUri);
        ...
        private static VerificationResult VerifyAdditionalSessionDataParams(PXService.Model.PXInternal.PaymentSession paymentSession, string paymentContext, string piId, List<string> exposedFlightFeatures, System.Uri requestUri = null)
        {
            try
            {
                ...
                // Add a new flight to Azure Exp titled "PXAdditionalVerificationRelaxEnforcement_<lowercasePartnerName> to skip additional validation step for specific partners
                if (!RelaxEnforcement(exposedFlightFeatures, paymentSession.Partner))
                {
                    if (((decimal)paymentContextObj?.Pretax == 0 && (decimal)paymentContextObj?.PostTax == 0) || 
                        (exposedFlightFeatures != null && exposedFlightFeatures.Contains(Flighting.Features.PXSkipAdditionalValidationForZeroAmount, StringComparer.OrdinalIgnoreCase) && paymentSessionAmount == 0)) 
                    {
        ...
        private static bool RelaxEnforcement(List<string> exposedFlightFeatures, string paymentSessionPartner)
        {
            string skipValidationFlight = "PXAdditionalVerificationRelaxEnforcement_" + paymentSessionPartner.ToLower();

            if (exposedFlightFeatures != null && exposedFlightFeatures.Contains(skipValidationFlight, StringComparer.OrdinalIgnoreCase))
            {
                return true;
            }
