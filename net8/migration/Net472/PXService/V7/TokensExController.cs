                    ...
                    foreach (PIDLResource pidlResource in challengeMethodTypesPidl)
                    {
                        ...
                        if (challengeTypeProperty != null && deviceBindingResponse.ChallengeMethods != null && challengeTypeDisplayHint != null)
                        {
                            ...
                            foreach (ChallengeMethod challengeMethod in deviceBindingResponse.ChallengeMethods)
                            {
                                ...
                                SelectOptionDescription option = new SelectOptionDescription();

                                GroupDisplayHint challengeMethodGroupHint = new GroupDisplayHint
                                {
                                    HintId = Constants.DisplayHintIdPrefixes.OptionTextContainer + challengeMethod.ChallengeMethodId,
                                };

                                string challengeMethodFriendlyName = null;
                                if (Constants.ChallengeMethodTypeToFriendlyNameMappings != null && 
                                    Constants.ChallengeMethodTypeToFriendlyNameMappings.TryGetValue(challengeMethod.ChallengeMethodType, out challengeMethodFriendlyName))
                                {
                                    // Use the found challengeMethodFriendlyName
                                }
                                else
                                {
                                    // Fallback to the enum value as string
                                    challengeMethodFriendlyName = challengeMethod.ChallengeMethodType.ToString();
                                }

                                TextDisplayHint challengeMethodType = new TextDisplayHint { DisplayContent = LocalizationRepository.Instance.GetLocalizedString(challengeMethodFriendlyName, language) };
                                TextDisplayHint cardHolderName = new TextDisplayHint { DisplayContent = challengeMethod.ChallengeValue };

                                challengeMethodGroupHint.Members.Add(challengeMethodType);
                                challengeMethodGroupHint.Members.Add(cardHolderName);

                                option.DisplayContent = challengeMethodGroupHint;

                                challengeTypeDisplayHint.PossibleOptions[challengeMethod.ChallengeMethodId] = option;
                            }
                        }

                        ButtonDisplayHint buttonDisplayHint = pidlResource.GetDisplayHintById("continueSubmitButton") as ButtonDisplayHint;
