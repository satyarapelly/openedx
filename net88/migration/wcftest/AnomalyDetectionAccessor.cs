    ...
    public class AnomalyDetectionAccessor : IAnomalyDetectionAccessor
    {
        ...
        private void CheckAndQueueBackgroundTaskToReadLatestBlobData(EventTraceActivity traceActivityId)
        {
            ...
            try
            {
                        ...
                        if (Interlocked.Exchange(ref this.checkBlobReadCounter, 1) == 0)
                        {
                            ...
                            HostingEnvironment.QueueBackgroundWorkItem(async cancellationToken =>
                            {
                                try
                                {
                                    ...
                                    SllWebLogger.TraceServerMessage("AnomalyDetectionAccessor", traceActivityId.ToString(), null, $"Got anomaly detection results. MaliciousAccountCount : {this.maliciousAccountIds?.Count}, MaliciousClientIPCount: {this.maliciousClientIPs?.Count}", Diagnostics.Tracing.EventLevel.Informational);
                                }
                                catch (Exception ex)
                                {
                                ...
                                {
                                    SllWebLogger.TraceServerMessage("AnomalyDetectionAccessor", traceActivityId.ToString(), null, "Setting blob read counter to 0", Diagnostics.Tracing.EventLevel.Warning);
                ...
                {
                    SllWebLogger.TraceServerMessage("AnomalyDetectionAccessor", traceActivityId.ToString(), null, $"blodClients didn't exist or not a HostedEnvironment", Diagnostics.Tracing.EventLevel.Warning);
                }
            }
            catch (Exception ex)
            {
