...
namespace Microsoft.Commerce.Payments.PXService
{
    ...
    public class PXServiceApiVersionHandler : DelegatingHandler
    {
        ...
        protected override async Task<HttpResponse> SendAsync(HttpRequest request, CancellationToken cancellationToken)
        {
            ...
            string ipAddress = this.GetUserIpAddress(request);
            string country = null;
                        ...
                        foreach (string riskData in riskInfoList)
                        {
                            if (!string.IsNullOrEmpty(riskData) && riskData.StartsWith("referrer=", StringComparison.OrdinalIgnoreCase))
                            {
                                    ...
                                    && string.Equals(clientContextEncoding, "base64", StringComparison.OrdinalIgnoreCase))
                                {
                                    ...
                                    var base64EncodedBytes = System.Convert.FromBase64String(encodedReferrer);
            ...
            Flighting.FeatureConfig featureConfig = this.settings.AzureExpEnabled ? await this.settings.AzureExPAccessor.GetExposableFeatures(flightContext, traceActivityId) : null;
            ...
            exposableFeatures = LocalFeatureConfigs.MergeMatchedEligibleLocalAndRemoteFeatures(
                ...
                partner: partner,
                country: country,
                localFeatureConfigs: this.settings.LocalFeatureConfigs,
            ...
            PartnerSettings partnerSettings = await PartnerSettingsHelper.GetPaymentExperienceSetting(this.settings, partner, partnerSettingsVersion, traceActivityId, exposableFeatures);
        ...
        private string GetUserIpAddress(HttpRequest request)
        {
            ...
            try
            {
                ...
                ContextHelper.GetContext(request, this.settings, ref context);
